...
struct ExternalEvaluation : public Norm {
	...
	void operator() (const Input& solution, Value& result, double& violation, Param& parameters) const {
		...
		if (outRes != 0)
			throw (std::string("Failed to run ")+executableCommand + " " + homeDir).c_str();
//		std::cout << (executableCommand + " " + homeDir) << "\n";
	...
	void writeIn(const Input& solution) const {
		std::ofstream file((homeDir != "" ? (homeDir + "/" + inFname).c_str() : inFname.c_str()));
		if (file) {
		...
		if (!file) 
			throw std::runtime_error("ExternalEvaluation: writing to file failed");
	}
